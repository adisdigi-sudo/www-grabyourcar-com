 ...
 // Google Reviews data structure (demo data - ready for API integration)
 ...
 // Demo Google Reviews data (replace with API data when integrated)
 const googleReviews: GoogleReview[] = [
   {
     id: "1",
     authorName: "Rahul Sharma",
     rating: 5,
     text: "Excellent service! Got my Hyundai Creta at the best price in Mumbai. The team was very professional and handled all the paperwork smoothly. Highly recommend GrabYourCar for hassle-free car buying.",
   ...
   },
   {
     ...
     rating: 5,
     text: "Amazing experience buying my first car through GrabYourCar. They helped me compare multiple variants of Tata Nexon and got me the best finance option. Doorstep delivery was cherry on top!",
     relativeTime: "1 month ago",
     isLocalGuide: false,
   },
   {
     id: "3",
     authorName: "Amit Kumar Singh",
     rating: 4,
     text: "Good experience overall. The price was competitive and the team was helpful. Waiting time for XUV700 was long but that's expected. Would have given 5 stars if communication was more frequent.",
   ...
   },
   {
     ...
     rating: 5,
     text: "Second purchase from GrabYourCar. Got a Swift for my parents last year and now Kia Seltos for myself. They always deliver on their promise of best deals. Trusted platform!",
   ...
   },
   {
     ...
     rating: 5,
     text: "The hybrid Maruti Grand Vitara was hard to find anywhere, but GrabYourCar located one quickly. Great pricing on insurance too. Very satisfied with the entire process.",
     relativeTime: "1 month ago",
     isLocalGuide: true,
   },
   {
     ...
     rating: 4,
     text: "Bought Toyota Innova Hycross. The wait was long but the team followed up consistently. Good savings compared to direct dealer price. Professional service.",
     relativeTime: "2 months ago",
     isLocalGuide: false,
   },
 ];
 
 // Business stats from Google
 const businessStats = {
   rating: 4.6,
   totalReviews: 142,
   responseRate: "100%",
   ...
   >
     <Card className="p-5 h-full bg-card border border-border/50 hover:border-primary/20 hover:shadow-lg transition-all duration-300 group">
       ...
       <div className="flex items-start gap-3 mb-3">
         <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 shadow-md">
       ...
       {/* Review Text */}
       <p className="text-foreground/90 text-sm leading-relaxed line-clamp-4 group-hover:line-clamp-none transition-all duration-300">
         {review.text}
       </p>
 ...
 export const Testimonials = () => {
   const handleViewAllReviews = () => {
     // Replace with actual Google Maps URL when Place ID is available
       ...
       <div className="container mx-auto px-4">
             ...
             <div className="text-left">
               <div className="flex gap-0.5 mb-1">
                 {Array.from({ length: 5 }).map((_, i) => (
                   <Star 
                     key={i} 
                     className={`h-5 w-5 ${i < Math.floor(businessStats.rating) ? 'fill-amber-400 text-amber-400' : i < businessStats.rating ? 'fill-amber-400/50 text-amber-400' : 'text-muted-foreground/30'}`} 
                   />
                 ))}
               </div>
         ...
         <div className="mt-10 md:mt-12 text-center">
           <Button
             ...
             variant="outline"
             className="gap-2 px-6 py-3 h-auto font-medium hover:bg-primary hover:text-primary-foreground transition-colors"
           >
             <svg className="h-4 w-4" viewBox="0 0 24 24">
