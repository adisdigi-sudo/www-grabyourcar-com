...
export function useCreateEmailTemplate() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (template: { name: string; subject?: string; html_content?: string; json_content?: Record<string, unknown>; category?: string; is_ai_generated?: boolean }) => {
      const { data, error } = await supabase.from("email_templates").insert({ name: template.name, subject: template.subject, html_content: template.html_content, json_content: template.json_content as any, category: template.category || 'custom', is_ai_generated: template.is_ai_generated || false }).select().single();
...
export function useUpdateEmailTemplate() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...updates }: { id: string; [key: string]: unknown }) => { const { data, error } = await supabase.from("email_templates").update(updates).eq("id", id).select().single(); if (error) throw error; return data; },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["email-templates"] }),
  });
}
...
export function useEmailCampaigns() {
  return useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("email_campaigns").select("*, email_templates(name), email_lists(name)").order("created_at", { ascending: false }); if (error) throw error; return data; },
  });
}
...
export function useCreateEmailCampaign() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (campaign: { name: string; subject?: string; from_name?: string; from_email?: string; template_id?: string; list_id?: string; html_content?: string }) => {
