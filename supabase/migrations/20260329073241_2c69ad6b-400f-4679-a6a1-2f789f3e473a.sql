...
BEGIN
  ...
  IF v_is_stage_change AND (
    OLD IS NULL 
    OR NEW.booking_date IS NOT DISTINCT FROM OLD.booking_date
    OR NEW.booking_date < (CURRENT_DATE - INTERVAL '60 days')::date
  ) THEN
  ...
  ELSE
    v_booking_date := COALESCE(
      CASE 
        WHEN NEW.booking_date IS NOT NULL 
             AND NEW.booking_date >= (CURRENT_DATE - INTERVAL '60 days')::date 
  ...
  IF v_existing_policy_id IS NOT NULL THEN
    ...
    SET
      ...
      issued_date = COALESCE(
        CASE WHEN v_is_stage_change THEN v_booking_date ELSE issued_date END,
        issued_date, 
        v_booking_date
      ),
