...
-- Update the trigger function to handle the unique constraint gracefully
...
BEGIN
  ...
  -- Skip if another client already has an active policy for this vehicle
  ...
  -- Find existing active policy for this client
  ...
  IF v_existing_policy_id IS NOT NULL THEN
    ...
    -- Mark any other active policies for this client as renewed
    UPDATE public.insurance_policies
  ...
  ELSE
    -- Mark existing active policies as renewed before inserting new one
    UPDATE public.insurance_policies
    SET status = 'renewed', renewal_status = 'renewed', updated_at = now()
    WHERE client_id = NEW.id AND status = 'active';

    -- Insert new policy (with conflict handling for the unique index)
